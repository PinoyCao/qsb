...
public class ReadMemoryImage {
     ...
     *             当读写错误或不识别的格式时抛出
     */
    public static final BufferedImage readMemoryImage(InputStream imgStream) throws IOException {
        if (null == imgStream )
            throw new NullPointerException("the argument 'imgStream' must not be null or empty");
        // 将字节数组转为InputStream，再转为MemoryCacheImageInputStream
        ImageInputStream imageInputstream = new MemoryCacheImageInputStream(imgStream);
